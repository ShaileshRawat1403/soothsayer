...
DAX (Data Agent eXecutor) provides the execution workstation with run management, approvals, and replay features.

### Setup

1. **Start DAX server** (external service):
   - Default URL: `http://127.0.0.1:4096`
   - Configure via `DAX_BASE_URL` in `apps/api/.env`

2. **Verify DAX is running**:
   ```bash
   curl http://127.0.0.1:4096/health
   ```

### Launch Soothsayer with DAX
...
- **DAX server not reachable**: Ensure `DAX_BASE_URL` is correct and DAX service is running
- **Run stuck on approval**: Approve/deny from the run console or Approval Inbox
- **Refresh recovery**: Works for active and approval-paused runs
