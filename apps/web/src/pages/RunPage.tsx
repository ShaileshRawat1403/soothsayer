      ...
      <div className="mx-auto flex w-full max-w-[1200px] flex-col gap-8 p-10 animate-in-up">
        <header className="flex items-center justify-between border-b border-border/40 pb-6">
          <div className="flex flex-col gap-1">
            <div className="flex items-center gap-2 text-[9px] font-black uppercase tracking-[0.3em] text-muted-foreground/60">
              <Activity className="h-3 w-3" />
              Direct Entry
            </div>
            <h1 className="text-3xl font-black tracking-tighter">Execution Launcher</h1>
          </div>
          <Link to="/terminal" className="button-professional border border-border/60 hover:bg-muted text-muted-foreground">
            Cancel
          </Link>
        </header>
        ...
        <div className="grid gap-10 lg:grid-cols-[1fr_300px]">
          <div className="space-y-8">
            <div className="space-y-3">
              <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60 ml-1">Instruction Buffer</label>
              <textarea
                value={launchInput}
                onChange={(e) => setLaunchInput(e.target.value)}
                placeholder="Specify execution path..."
                className="min-h-48 w-full rounded-2xl border border-border/60 bg-card/20 px-6 py-5 text-sm font-medium focus:ring-4 focus:ring-primary/5 focus:border-primary/20 outline-none transition-all placeholder:text-muted-foreground/20"
              />
            </div>
                ...
                {(['general', 'analysis', 'edit'] as const).map((kind) => (
                  <button
                    ...
                    className={cn(
                      ...
                      launchKind === kind ? "bg-background text-foreground shadow-sm" : "text-muted-foreground/60 hover:text-foreground"
                    )}
                  >
                    {kind}
                  </button>
          ...
          <aside className="space-y-6">
            <div className="rounded-2xl border border-border/40 bg-card/30 p-6 space-y-6">
              <div className="space-y-2">
                <label className="text-[9px] font-black uppercase tracking-widest text-muted-foreground/60">Persona Sync</label>
                <div className="relative group">
                  <select
                    value={selectedPersonaId}
                    onChange={(e) => setSelectedPersonaId(e.target.value)}
                    className="w-full appearance-none rounded-xl border border-border bg-background px-4 py-3 text-xs font-black focus:ring-4 focus:ring-primary/5 outline-none transition-all"
                  >
                    {personas.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
                  </select>
                  <ChevronDown className="absolute right-4 top-1/2 -translate-y-1/2 h-3.5 w-3.5 text-muted-foreground/40" />
                </div>
              </div>
              <div className="pt-4 border-t border-border/40">
                <div className="flex justify-between items-center text-[9px] font-black uppercase tracking-widest">
                  <span className="text-muted-foreground/40">Authority</span>
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-background">
        <div className="flex flex-col items-center gap-4">
          ...
          <span className="text-[9px] font-black uppercase tracking-[0.4em] text-muted-foreground/40">Syncing Node</span>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto flex w-full max-w-[1600px] flex-col gap-6 p-6 animate-in-up">
      <header className="flex items-center justify-between px-2">
        <Link to="/dax" className="flex items-center gap-2 text-[9px] font-black uppercase tracking-widest text-muted-foreground hover:text-foreground transition-colors group">
